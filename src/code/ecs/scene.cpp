...
Entity* Scene::GetEntity(long long uuid) const noexcept {
	for (auto &entt : m_entities)
	{
...
Entity* Scene::GetEntityWithUUID(long long uuid) const noexcept {
	for (auto &entt : m_entities)
	{
		if (entt->GetUUID() == uuid)
		{
...
Entity* Scene::GetEntityWithUUID(const std::string& uuidstring) const noexcept {
	for (auto &entt : m_entities)
	{
...
Entity* Scene::GetEntity(std::string name, long long uuid) const noexcept {
	for (auto &entt : m_entities)
	{
...
Entity* Scene::AddEntity(std::string name, long long uuid, int tag, int layer) {
	const auto& entity = m_entities.emplace_back(new Entity);
	entity->SetName(name);
	entity->SetUUID(uuid);
	entity->SetTag(tag);
...
void Scene::Deserialize(const std::string &filePath) {
	...
	if (entities)
	{
		for (auto entity : entities)
		{
			...
			Entity* newEntity = AddEntity(name, uuid, tag, layer);
