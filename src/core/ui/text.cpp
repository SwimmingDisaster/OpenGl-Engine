...
void Font::Init(std::string fileName, std::string textureName) {
	...
	for (int i = 0; i < num; i++)
	{
		...
		int id = stoi(strs[2]);
		float x = (float)stoi(strs[4]) / 512.0f; 		float y = (float)stoi(strs[6]) / 512.0f;
		float sizeX = (float)stoi(strs[8]) / 512.0f; 	float sizeY = (float)stoi(strs[10]) / 512.0f;
		float xMax = x + sizeX; 						float yMax = y + sizeY;
		float xOffset = (float)stoi(strs[12]) / 512.0f; float yOffset = (float)stoi(strs[14]) / 512.0f;
